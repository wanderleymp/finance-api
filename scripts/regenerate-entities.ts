...
async function regenerateEntities() {
  ...
  try {
    ...
    for (const table of relevantTables) {
      ...
      const generateColumnDecorator = (column: any) => {
      ...
      const mapTypeOrmType = (dataType: string, column: any) => {
        switch (dataType) {
          case 'integer':
          case 'smallint':
          case 'bigint':
      ...
      const generatePrimaryKeyDecorator = (column: any) => {
        if (column.column_name.toLowerCase().endsWith('_id') || column.column_name === 'id') {
      ...
      const generateAutoIncrementDecorator = (column: any) => {
      ...
      const generateCreateDateDecorator = (column: any) => {
      ...
      const generateUpdateDateDecorator = (column: any) => {
