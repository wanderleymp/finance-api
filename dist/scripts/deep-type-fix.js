            ...
            const properties = node.properties.map(prop => {
                if (ts.isPropertyAssignment(prop)) {
                    ...
                    if (name === 'user_name' && ts.isIdentifier(prop.initializer)) {
                        return ts.factory.createPropertyAssignment(prop.name, ts.factory.createStringLiteral(prop.initializer.getText()));
                    }
...
function processTypeScriptFile(filePath) {
    ...
    if (sourceFile) {
        ...
        const result = printer.printNode(ts.EmitHint.SourceFile, transformedSourceFile, sourceFile);
...
//# sourceMappingURL=deep-type-fix.js.map